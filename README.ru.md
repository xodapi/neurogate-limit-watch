...
по окнам 5 часов, 24 часа, 7 дней и 30 дней, включает live-monitor в стиле
`abtop`, а также может добавить локальный
...
`nglimit-open.cmd`: helper для двойного клика, который всегда делает паузу, и
...
- Полноэкранный live-monitor с выходом через `q`/`Esc` и ручным refresh через
  `r`.
